...
Provide an opt-in Agentic RAG self-correction loop that wraps the retrieve → generate cycle for enabled `(tenant, intent)` pairs: it scores each candidate answer with an online quality gate, and on failure rewrites the query (HyDE) and re-retrieves within a bounded deadline and iteration budget, returning the best-so-far candidate with a low-confidence flag when no candidate clears the thresholds. The loop is controlled by static environment configuration and degrades gracefully to the single-pass path when its scoring backend is unavailable.
