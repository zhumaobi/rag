...
- [ ] **HyDE（Hypothetical Document Embeddings）**：先让 LLM 预先生成假设性答案，再用答案的嵌入去检索，缓解 query-document 语义鸿沟。
...
### Agentic RAG
- [ ] **Agentic RAG**：引入 Agent 循环——按需决定是否检索、检索什么、是否需要多轮补充检索，支持工具调用与多数据源路由。
...
- [ ] **答案自检与幻觉检测**：基于 RAGAs 的 faithfulness 在线校验，低分触发降级或重生成。
